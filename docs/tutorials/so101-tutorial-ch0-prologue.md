...
在进入具体路线之前，必须先理解一个核心概念——机器人学习涉及**三个不同的环境**，它们各自独立、可以不同，而它们之间的匹配关系直接决定了模型能不能成功。

### 三个环境

```
┌──────────────────┐         ┌──────────────────┐         ┌──────────────────┐
│  数据采集环境      │  录制    │  训练过程          │  训练    │  评测环境          │
│                  │ ──────→ │                  │ ──────→ │                  │
│  谁在操作？       │  数据    │  模型学什么？      │  模型    │  模型考什么？      │
│  真机 or 仿真     │         │  数据中的模式      │         │  在新环境里执行     │
└──────────────────┘         └──────────────────┘         └──────────────────┘
```

**① 数据采集环境**：演示数据的来源。可以是真机（人遥操作录制），也可以是仿真（脚本自动生成）。这个环境决定了模型"看到过什么"。

**② 训练过程**：模型从数据中学习"图像→动作"的映射。训练本身不关心环境长什么样，它只学习数据中的模式。

**③ 评测环境**：模型被放进一个环境里实际执行任务，看能不能成功。环境的视觉外观、物理参数、相机视角都影响结果。

### 为什么三者的匹配至关重要

模型学到的是"在特定视觉条件下，看到某个物体时该怎么动"。如果评测环境和数据采集环境长得不一样——光照不同、背景不同、相机角度不同——模型就认不出物体，动作自然无效。

这就是**领域不匹配（Domain Mismatch）**：

| 匹配情况 | 结果 | 例子 |
|----------|------|------|
| 数据环境 = 评测环境 | ✅ 模型能泛化 | sim twin：MuJoCo 采集 → MuJoCo 评测 |
| 数据环境 ≠ 评测环境 | ❌ 模型失效 | ataghof：真机采集 → MuJoCo 评测 |
...
| 环境 | 实际情况 |
|------|---------|
| 数据采集 | ataghof 用**真机** SO-101 录制，相机拍的是真实桌面、真实光照 |
| 训练 | SmolVLA 学习真机照片中的"图像→动作"映射 |
| 评测 | 放进 **MuJoCo 仿真** so101_nexus 环境执行 |

结果：**0% 成功率**。模型在真机照片上学到的视觉特征，在 MuJoCo 渲染画面中完全失效。

**方案 B（成功）**：sim twin 数据集

| 环境 | 实际情况 |
|------|---------|
| 数据采集 | **MuJoCo 仿真**脚本自动生成 |
| 训练 | SmolVLA 学习仿真渲染图中的"图像→动作"映射 |
| 评测 | 放进**同一个 MuJoCo 仿真**环境执行 |

结果：**47% 成功率**。数据和评测来自同一个虚拟场景，视觉完全一致。
...
|----------|------|--------|------|
| 真机 | 真机 | ✅ | 最理想，但成本最高 |
| 真机 | 仿真 | ❌ | Sim-to-Real Gap（我们的方案 A） |
| 仿真 | 仿真（同场景） | ✅ | Sim-to-Sim（我们的方案 B） |
| 仿真 | 仿真（不同场景） | ❌ | 场景差异导致不匹配 |
| 仿真 | 真机 | ✅ | Sim-to-Real 迁移（终极目标） |

### 实用判断法则

在训练之前，用这个简单方法判断数据集和评测环境是否匹配：

> **可视化对比**：取数据集的一帧图像，和评测环境的渲染截图放在一起看。如果人眼能看出明显差异（光照、背景、物体外观），模型大概率也会失败。

这不是严格的科学方法，但在实践中非常有效——我们方案 A 的失败本可以通过这个简单的视觉对比提前发现。

---

## 5. 两条路线：纯仿真验证 vs Sim-to-Sim 验证
...
| Ch6 | LIBERO 评测实战 | SO101 自定义机器人 + LIBERO 多任务评测 |
| Ch7 | 算法对比 | PPO vs VLA vs ACT 横向对比 |
| 附录 | 环境速查 | 命令 + 配置 + 数据集对比 |
...
4. **如何在训练前判断一个公开数据集和你的评测环境是否匹配？**  
   提示：§4 的实用判断法则——取一帧数据图像和评测环境渲染截图做视觉对比。
