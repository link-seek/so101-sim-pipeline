...
47% 之后，我们把同模型续到 20K，想看看能不能更高——结果 145/325 = 45%，掉了 8 个 episode（[Discussion #20](https://github.com/link-seek/so101-sim-pipeline/discussions/20)）。同一把尺子（SO101 MuJoCo Grid Sweep，325 episodes），同一个模型，多训 5K 步，分数没涨。

这 8 个 episode 算不算"倒退"、加步数为什么没用、什么手段才有效——这些分析属于评测方法论，完整版在 [Ch5 §5.6](so101-tutorial-ch5-evaluation.md)：三行对照表、三句话解读、噪声带计算都在那里。本章只记录事实：**调好是答案，调坏也是答案**。
