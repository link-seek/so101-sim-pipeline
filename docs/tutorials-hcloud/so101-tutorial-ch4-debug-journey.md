...
08-19  Sim Twin 训练 + 评测 → 47% 成功！🎉
  │
09-05  续训 20K → 45%：噪声带内平台，加步数路线死（#20）
  │
09-07  手段对照：加对任务的数据 > 加步数（#18，详见尾声）
  │
09-08  Sawyer 100-ep 对照取消，路线已停 → 结论见 Ch7 §2.4
```
...
## 尾声：从 47 调到 45——续训调坏的完整记录

47% 不是终点。拿着调好的模型，我们又续训到 20K，想看看能不能更高——结果掉到 45%。调好和调坏都是答案，这一节把后半段补完。

三个数全是同一把尺子量的：SO101 MuJoCo Grid Sweep，5 reach × 13 azim × 5 trials = 325 episodes（[Discussion #18](https://github.com/link-seek/so101-sim-pipeline/discussions/18)）。

| 阶段 | Checkpoint / 数据 | 分数 | 含义 |
|------|-------------------|------|------|
| 错任务数据 | libero_object 30eps 训出的变体，15K | **0/325 = 0%** | 数据任务不对，网格全灭 |
| 对任务数据 | pick-cube 数据，15K | **153/325 = 47%** | 本章旅程终点，与历史 run（`32221378632`）逐数相同 |
| 单纯加步数 | 同模型续到 20K | **145/325 = 45%** | 掉 8 个 episode，不再涨 |

三句话解读：

1. **0 的教训**：换任务数据 ≠ 变体。pick-cube 的能力只从 pick-cube 的数据来。
2. **47 的含义**：对任务数据 + 15K，能复现历史分数。47% 是 325 个初始条件的平均：中心区域 60–100%，边缘接近 0%——单一数字掩盖了覆盖盲区。
3. **45 的诚实读法**：145 vs 153 只差 8 个 episode（2.5 个百分点）。在 N=325、p≈0.46 时标准误约 ±2.8%，这个差距**落在噪声带里**。结论只能写“平台/不再涨”，不能写“显著倒退”——加步数路线死，不是因为它显著变差，而是因为它**不再变好**。
...
> **本章 takeaway**：在数据欠拟合时，**加对任务的数据 > 加步数；步数只负责走到收敛，跨不过数据的天花板**。跨实验对比前先问“同一把尺子吗”（N、身体、任务分布任一不同，数字直接比就是耍流氓）。
>
...
| 信号 | 决策 |
|------|------|
| 加步数后分数在噪声带内波动（15K→20K，±2.8%） | 停训，判平台，不追 |
| 续训成本远超信息增益（~43h 卡时换一个已知答案） | 停，见尾声 |
...
5. **47% vs 45% 差 8 个 episode，为什么结论是"平台"而不是"倒退"？**  
   提示：N=325、p≈0.46 时标准误约 ±2.8%，2.5 个百分点的差距落在噪声带里。什么证据才能判显著倒退？

---

**本章小结**：我们用 `replay_demo.py` 和 `eval_mujoco_policy.py` 走完了从 0% 到 47% 再到 45% 的完整旅程——调好是答案，调坏也是答案。评测脚本的设计原理与社区标准框架，下一章系统介绍。
