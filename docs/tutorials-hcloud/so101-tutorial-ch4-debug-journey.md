...
08-19  Sim Twin 训练 + 评测 → 47% 成功！🎉
  │
09-05  同模型续到 20K → 45%，没更高（#20，判定见本章尾声）
```
...
这 8 个 episode 算不算"倒退"、加步数为什么没用、什么手段才有效——判定直接写在这里，同一把尺子量出的三个数：

> 数据来源：[Discussion #20](https://github.com/link-seek/so101-sim-pipeline/discussions/20)（2026-09-05）、[#18](https://github.com/link-seek/so101-sim-pipeline/discussions/18)（2026-09-07）。三个数**全是同一把尺子量的**：SO101 MuJoCo Grid Sweep，5 reach × 13 azim × 5 trials = 325 episodes。
...
2. **47 的含义**：对任务数据 + 15K，能复现历史分数。47% 是 325 个初始条件的平均：中心区域 60–100%，边缘接近 0%（见本章方案 B 分析小节）——单一数字掩盖了覆盖盲区。
3. **45 的诚实读法**：145 vs 153 只差 8 个 episode（2.5 个百分点）。N=325、p≈0.46 时标准误约 ±2.8%（`SE = sqrt(p*(1-p)/N)`；N 是网格乘出来的 5×13×5，不是按公式选的，事后验证恰好够用）。差距**落在噪声带里**，结论只能写"平台/不再涨"，不能写"显著倒退"。记住一条：**报成功率不报 N，就是耍流氓**。
...
> **尾声 takeaway**：在数据欠拟合时，**加对任务的数据 > 加步数；步数只负责走到收敛，跨不过数据的天花板**。跨实验对比前先问"同一把尺子吗"（N、身体、任务分布任一不同，数字直接比就是耍流氓）。
>
> **路线已停说明**：Sawyer 50→100ep 对照（10eps/task）于 2026-09-08 取消——10eps/task 仍远低于社区配方密度（50eps/单task），预期买不到能改变结论的数据，而重训需 ~43h 卡时（实测 8s/step，数据加载瓶颈），性价比不足。结论以 50-ep + ACT 为准（详见 Ch7 §2.4）。本章只记录事实：**调好是答案，调坏也是答案**——现在判定也有了：45% 不是倒退，是平台。
...
**本章小结**：我们用 `replay_demo.py` 和 `eval_mujoco_policy.py` 走完了从 0% 到 47% 再到 45% 的完整旅程——调好是答案，调坏也是答案，45% 的统计判定见本章尾声。
