...
| 指标 | v1 (5cm) | v2 (15cm) | 直跑复现 #20 (15cm) |
|------|----------|-----------|---------------------|
| best_success | 0.995 | 0.9925 | 0.973 |
| eval success_rate | **100%** | **98%** (49/50) | **100%** (50/50) |
| avg_reward | 1.440 | 1.511 | 1.537 |
| avg_steps | 50.0 | 58.4 | 47.2 |

> 已验证（Discussion #20）：修正后教程命令（`scripts/` 路径 + 短横参数）独占跑完 30M steps，50/50。
> 附带教训：与 SmolVLA 并行训练时同一命令收敛崩到 eval 0.02——评测机上禁止并行训练任务。
