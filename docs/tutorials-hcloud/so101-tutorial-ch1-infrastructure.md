...
训练和评估在同一容器内顺序执行。一个镜像完成训练-评估闭环。归档有两条路：人肉 `obsutil`（§5），或走流水线全自动（GH Actions 已验证，CodeArts 待建，见 #19）——都不在这条命令里。
...
两条路：本节是人肉 `obsutil` 手工归档；跑流水线（GH Actions / CodeArts）时归档是自动的（upload 步骤），不用敲这些命令。
