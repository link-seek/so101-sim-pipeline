...
整个训练-评估流程很简单：**一台云服务器 + 一个 Docker 镜像 + 一条命令**（HF 拉数据→训练→评估）。全自动闭环（含 OBS 归档、开关机）是流水线的功能，不是这条命令的——见 §5 与 Ch0 §3.3。
...
```
数据集 (HF Hub，训练时自动下载) ──→ Docker 容器 ──→ 训练脚本 ──→ checkpoint ──→ 评估脚本 ──→ 评估结果
```

训练和评估在同一容器内顺序执行。一个镜像完成训练-评估闭环。归档有两条路：人肉 `obsutil`（§5），或走交付流水线全自动（CodeArts `franka-eval-pipeline`，已交付并跑通，见 #19）——都不在这条命令里。
...
两条路：本节是人肉 `obsutil` 手工归档；跑交付流水线（CodeArts）时归档是自动的（upload 步骤），不用敲这些命令。
