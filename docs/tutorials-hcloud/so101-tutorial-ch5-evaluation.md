...
### 4.8 跨身体实战：SO101 模型跑 Franka 考场

LIBERO 真题卷的一次跨身体实测：SO101 的模型，Franka 的考场。（全教程 LIBERO 系结果总览在 §5 进展表；Franka 原生线见 Ch6，SO101 集成线见 Ch8。）
...
| 325 | ±5.5% | 可接受——事后看恰好够用（N 是网格乘出来的：5 reach × 13 azim × 5 trials，公式只验证它够用） |
| 1000 | ±3% | 好，但 V100 跑不起 |

**我们的选择**：PPO 50eps（策略稳定，100% 或 0%，SE≈0）；VLA 沿用 grid sweep 的 325（网格乘积，事后验证统计够用）；LIBERO 按官方协议（10eps/task，Franka 线见 Ch6 §5.2）。记住一条：**报成功率不报 N，就是耍流氓**——§5.6 的"平台≠倒退"全靠这条才立得住。
