...
**Sawyer 专模线 verdict（#17 收尾，2026-09-08）**：上面是“Franka 模型 × Sawyer 身体”（跨身体，0% 预期内）。我们还跑了“**Sawyer 专模 × Sawyer 身体**”（2x2 右下角）：专家采集 `libero_spatial` 10 tasks × 5 eps = 50 eps（7141 帧）→ 微调 SmolVLA-Sawyer 20K @ batch 8（5K/10K/15K/20K 全 ckpt，final loss 0.034）+ ACT 20K 对照（loss 0.127）→ Task0 10eps 评测**全部 0%**（SmolVLA gripper 恒开 0.02~0.12；ACT 首 action 合理、100 步开环漂移，denorm 已验证无误）。

结论是**数据问题**，不是调参问题：5eps/task 多任务 + ~7s 快演示 + ~22 epochs 过拟合（10K 反而好于 20K，与社区 #3287 同 pattern）。**路线停止，不再重训**（含 100-ep 重训：2026-09-08 在 3%/600step 处停止，8s/step，跑满需 ~43h，性价比不足）。2x2 右下角定格为 **0%（数据不足）**——恰好与左上 Ch6 的 47% 对照：分数由“模型—身体” pair 决定，管线本身只是尺子。
...
| Sawyer 专模（50-ep） → Sawyer 评测 | 0%（数据不足，路线已停，见 §2.4） |
